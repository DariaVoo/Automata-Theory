...
from parse.parse_file import parse_file, get_stack_symb_alphabet, main_make_transitions, make_transitions, MARKER_STACK

if __name__ == '__main__':
    rules, main_rules = parse_file('test2.txt')
    alphabet, stack_symb = get_stack_symb_alphabet(rules)

    transitions = make_transitions(rules, alphabet)
